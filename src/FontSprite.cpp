...
F2C::Rect FontSprite::getStringSize(std::string str) const {
    ...
    //current width
    F2C::uint w = this->char_rect[static_cast<size_t>(str[0])].width*this->zoom_x; //first letter
    //current height
    F2C::uint h = this->char_rect[static_cast<size_t>(str[0])].height*this->zoom_y; //first letter

    for(size_t i = 0;i < str.length();++i){
        if(str[i] == '\n'){
            h += this->char_rect[static_cast<size_t>(str[i])].height*this->zoom_y;
            x = 0;
            continue;
        }
        x += this->char_rect[static_cast<size_t>(str[i])].width*this->zoom_x;
