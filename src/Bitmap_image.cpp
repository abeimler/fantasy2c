...
bool Bitmap::loadFileintoBitmap(GLuint image_id,std::istream& file,F2C::Image::ImageType type,LogError* error){
    ...
    if(!file.good()){
		error->setError("C++ file-stream(std::istream) good() function returns false (possibly error by loading) \n");
		error->setErrorCode(ErrorCode::FILESTREAM_ERROR);
		isError = true;
	}else{
        file.seekg(0, std::ios_base::end);
        sizef = file.tellg();

        lump = new ubyte[sizef]();

        file.seekg(0, std::ios_base::beg);
        file.read((char*)lump,sizef);
	}
	if (!isError && !ilLoadL(static_cast<ILenum>(type), lump, sizef)){
	    if(error){
            error->setError(iluErrorString(ilGetError()));
            error->setErrorCode(ErrorCode::OPENIL_ERROR);
        }
		isError = true;
	}
	if(lump)
...
bool Bitmap::loadFileintoBitmap(GLuint image_id,FILE* file,F2C::Image::ImageType type,LogError* error){
    ...
    if (!file && error){
		error->setError("C file-stream(FILE*) is NULL (possibly error by loading) \n");
		error->setErrorCode(ErrorCode::FILESTREAM_ERROR);
		isError = true;
	}else{
...
bool Bitmap::loadImageintoBitmap(GLuint image_id,std::string filename,F2C::Image::ImageType type,LogError* error){
    ...
    if(!tmpFile.is_open()){
        if(error){
            ...
            buffer << "No such file or directory: " << filename.c_str() << std::endl;
