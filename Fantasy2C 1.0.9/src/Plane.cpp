	...
	viewport(NULL)
{
    if(!Plane::checkVertexArray){
        Plane::VertexArray = uglExtensionSupported("GL_EXT_vertex_array");
...
int Plane::render() const{
    ...
    GLint clippedRight = this->x + bgrect.width;
    GLint clippedBottom = this->y + bgrect.height;

    if(this->clip_rect){
        clippedLeft = std::max<int>(this->clip_rect->x,this->x);
        clippedTop = std::max<int>(this->clip_rect->y,this->y);
        clippedRight = std::min<uint>(this->x + bgrect.width,this->clip_rect->x + this->clip_rect->width);
        clippedBottom = std::min<uint>(this->y + bgrect.height,this->clip_rect->y + this->clip_rect->height);
    }
    ...
    if(sizex == 1 && sizey == 1){
        ...
        for(size_t iy = 0;iy < sizey;iy++){
            for(size_t ix = 0;ix < sizex;ix++){
                ...
                *                    width,height,   x,height}
                */
                size_t coordit = (ix+iy*sizex)*8;
                size_t coordiv = (ix+iy*sizex)*12;

//                texcoord[coordit] = (ix > 0)? fullcx : cx;
//                texcoord[1+coordit] = (iy > 0)? fullcy : cy;
//                texcoord[2+coordit] = (ix > 0)? fullcwidth : cwidth;
//                texcoord[3+coordit] = (iy > 0)? fullcy : cy;
//                texcoord[4+coordit] = (ix > 0)? fullcwidth : cwidth;
//                texcoord[5+coordit] = (iy > 0)? fullcheight : cheight;
//                texcoord[6+coordit] = (ix > 0)? fullcx : cx;
//                texcoord[7+coordit] = (iy > 0)? fullcheight : cheight;
//
//                vertices[coordiv] = (ix > 0)? vwidth+fullvwidth*(ix-1) : 0.0f;
//                vertices[1+coordiv] = (iy > 0)? vheight+fullvheight*(iy-1) : 0.0f;
//                vertices[2+coordiv] = 0.0f;
//
//                vertices[3+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix) : vwidth;
//                vertices[4+coordiv] = (iy > 0)? vheight+fullvheight*(iy-1) : 0.0f;
//                vertices[5+coordiv] = 0.0f;
//
//                vertices[6+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix) : vwidth;
//                vertices[7+coordiv] = (iy > 0)? vheight+fullvheight*(iy) : vheight;
//                vertices[8+coordiv] = 0.0f;
//
//                vertices[9+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix-1) : 0.0f;
//                vertices[10+coordiv] = (iy > 0)? vheight+fullvheight*(iy) : vheight;
//                vertices[11+coordiv] = 0.0f;

                texcoord[coordit] = fullcx;
                texcoord[1+coordit] = fullcy;
                texcoord[2+coordit] = fullcwidth;
                texcoord[3+coordit] = fullcy;
                texcoord[4+coordit] = fullcwidth;
                texcoord[5+coordit] = fullcheight;
                texcoord[6+coordit] = fullcx;
                texcoord[7+coordit] = fullcheight;

                vertices[coordiv] = (ix > 0)? vwidth+fullvwidth*(ix-1)-srect.x : 0.0f-srect.x;
                vertices[1+coordiv] = (iy > 0)? vheight+fullvheight*(iy-1)-srect.y : 0.0f-srect.y;
                vertices[2+coordiv] = 0.0f;

                vertices[3+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix)-srect.x : vwidth-srect.x;
                vertices[4+coordiv] = (iy > 0)? vheight+fullvheight*(iy-1)-srect.y : 0.0f-srect.y;
                vertices[5+coordiv] = 0.0f;

                vertices[6+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix)-srect.x : vwidth-srect.x;
                vertices[7+coordiv] = (iy > 0)? vheight+fullvheight*(iy)-srect.y : vheight-srect.y;
                vertices[8+coordiv] = 0.0f;

                vertices[9+coordiv] = (ix > 0)? vwidth+fullvwidth*(ix-1)-srect.x : 0.0f-srect.x;
                vertices[10+coordiv] = (iy > 0)? vheight+fullvheight*(iy)-srect.y : vheight-srect.y;
                vertices[11+coordiv] = 0.0f;
            }
        }
    }
