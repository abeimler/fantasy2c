...
TTFFont::TTFFont(std::string _filename,uint _size,const Color& _color,uint _charsperwidth,uint _charsperheight):
	filename(""),
	size(0),
	heights(256),
	widths(256)
{
    try{
        TTFFont::open(this->bitmap,_filename.c_str(),_size,_color,_charsperwidth,_charsperheight);
    }catch (LogError& err){
        throw err;
    }
...
void TTFFont::loadFile(std::string _filename ,uint _size,const Color& _color,uint _charsperwidth,uint _charsperheight){
    this->filename = "";
	this->size = 0;
