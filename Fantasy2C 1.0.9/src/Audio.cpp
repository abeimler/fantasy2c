...
AudioFile& AudioFile::operator = (const AudioFile& copy){
    if(this != &copy){
        ...
        if(this->device){
            this->sound = audiere::OpenSound(AudioFile::device, copy.filename.c_str(), true);
        }
        //throw exception, if No such file or directory File
        if(!this->sound){
            std::ostringstream buffer;
            LogError error;
            buffer << "No such file or directory: " << copy.filename.c_str() << '\n';
            error.setError(buffer.str());
            throw error;
        }
        if(copy.sound)
    ...
    volume(1.0f)
{
    ...
    //Try to open a sound buffer
    if(this->device){
        this->sound = audiere::OpenSound(AudioFile::device, _filename.c_str(), true);
    }
