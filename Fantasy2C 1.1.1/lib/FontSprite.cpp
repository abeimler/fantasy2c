...
int FontSprite::render() const{
    ...
    for(size_t i = 0;i < this->strings.size();i++){
        ...
        for(size_t j = 0;j < strlength;j++){
            ...
            if(this->flippedH){
                std::swap(texcoord[j*8],texcoord[2+j*8]);
                std::swap(texcoord[4+j*8],texcoord[6+j*8]);
            }
            if(this->flippedV){
                std::swap(texcoord[1+j*8],texcoord[7+j*8]);
                std::swap(texcoord[3+j*8],texcoord[5+j*8]);
            }
