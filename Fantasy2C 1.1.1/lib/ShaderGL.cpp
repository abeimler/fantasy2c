...
ShaderGL& ShaderGL::operator= (const ShaderGL& copy){
    if(this != &copy){
        if(!copy.fsCode.empty())
            this->LoadGLFragmentShader(copy.fsCode,NULL);
        if(!copy.vsCode.empty())
            this->LoadGLVertexShader(copy.vsCode,NULL);

        if(copy.fsCode.empty() && copy.vsCode.empty()){
            if (this->fragmentShader)
                glDeleteObjectARB( this->fragmentShader );
            if (this->vertexShader)
                glDeleteObjectARB( this->vertexShader );
...
int ShaderGL::LoadGLVertexShader(std::string shaderfilename,std::string* error){
    /* The vertex shader */
	std::string vsSource;
...
int ShaderGL::LoadGLFragmentShader(std::string shaderfilename,std::string* error){
	/* The fragment shader */
	std::string fsSource;
