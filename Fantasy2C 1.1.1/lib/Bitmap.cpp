...
void Bitmap::scale(uint _width,uint _height){
	...
	memset(_srcnewpixels,0,this->width * this->height * (this->bits >> 3)*sizeof(ubyte));
	memset(_dstnewpixels,0,_width * _height * (this->bits >> 3)*sizeof(ubyte));
...
void Bitmap::fillRect(int _x,int _y,uint _width,uint _height,uint8 r,uint8 g,uint8 b,uint8 a){
    ...
    for (size_t y = left; y < right; y++){
        for (size_t x = top; x < bottom; x++){
            ...
            //Alpha Blending
            //src_color * alpha + dst_color * (255-alpha) / 255
            //... >> 8 = ... / 256
            //=>  ((src_color * alpha) + (dst_color * (256-alpha))) >> 8 = 1..256
            //=> (((src_color * alpha) + (dst_color * (257-alpha))) >> 8) - 1 = 0..255

            //if: bpp >= 4 and dst_alpha > 0 && src_alpha < 255
            if(bpp >= 4 && *(dst+3) > 0 && a < 255){
                *dst = ((((r+1) * (a+1)) + ((*dst) * (257-a))) >> 8) - 1;
                *(dst+1) = ((((g+1) * (a+1)) + ((*(dst+1)) * (257-a))) >> 8) - 1;
                *(dst+2) = ((((b+1) * (a+1)) + ((*(dst+2)) * (257-a))) >> 8) - 1;
                *(dst+3) = ((((a+1) * (a+1)) + ((*(dst+3)) * (257-a))) >> 8) - 1;
            }else{
                *dst = r;
                *(dst+1) = g;
                *(dst+2) = b;
