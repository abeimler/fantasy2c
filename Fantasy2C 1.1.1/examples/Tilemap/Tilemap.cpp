...
void Tilemap::loadTileset(const Tileset& tileset){
    ...
    for(size_t i = 0;i < this->tileset.anitiles.size();i++){
        this->sprAniTiles.push_back( ArraySpriteAnimation (&this->tileset.anitiles[i].tile));
        this->sprAniTiles.back().setFrameperWidth(this->tileset.getTileWidth());
...
void Tilemap::deleteAniTileid(int x,int y,int layer){
    if(x < 0 || y < 0 || layer < 0 || x > this->width || y > this->height || layer > this->layers)
        return;
